    ...
    while True:
        ...
        # Fit scaler after first 6 samples
            ...
            if scaler_fitted and len(cpu_usage_history) > HISTORY_STEPS:
                ...
                print(f"CPU after {(PREDICTED_INDEX+1)*10}s={avg_predicted}")
                ...
                if pod_count>0:
                    ...
                    if rp_predicted>0.206:
                        ...
                        if predicted_pod_count>7:
                            ...
                            if not IS_SCALING:
                                temp_pod_count=min(predicted_pod_count,7)
                                t = threading.Thread(target=scale_deployment, args=(temp_pod_count,))
                                t.start()
                    
                            else:
                                print("CANT SCALE UP.ON GOING SCALING.")
                    elif pod_count>1:
                        if not IS_SCALING:
                            print('LETS SCALE DOWN')
                            t = threading.Thread(target=scale_deployment, args=(pod_count-1,))
                            t.start()  
        
                        else:
                            print("CANT SCALE DOWN. ON GOING SCALING.")  
                    else:
                        print('No Scaling')      
            ...
            print("SCALING ALREADY IN PROGRESS")
        time.sleep(10)
