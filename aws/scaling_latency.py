...
def scaleNodes(reqNodeCount):
    try:
        ...
        if total_nodes<desired_node_count:
            ...
            time_z_minus_y = (time_z - time_y).total_seconds()
            time_z_minus_x = (time_z - time_x).total_seconds()
            down.append({'downscaling_nodes':time_y_minus_x,'downscaling_pods':time_z_minus_y,'total_time':time_z_minus_x,'start':total_nodes,'end':desired_node_count})
...
def main():
    ...
    down=[]
    try:
        for additional_nodes in range(1,7): #adding 1 to 6 nodes
            for experiment_count in range(1,4): #5 experiments
                scaleNodes(additional_nodes+1)
                scaleNodes(1)
                time.sleep(120)  # just to make sure after downscaling we have a stable state. This is important
    except:
        print("Exception occurred!!")
    upscale = pd.DataFrame(up)
    downscale = pd.DataFrame(down)

    upscale.to_csv("upscaling.csv")
    downscale.to_csv("downscaling.csv")
